...
export const LikePostButton = ({ slug }: { slug: string }) => {
    const { isLoading, likePost, currentUserLikes } = usePostLikes(slug);
    ...
    return <button className="my-2" onClick={likePostHandler}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className={`h-6 w-6 hover:scale-110 transition-all ${currentUserLikes ? "fill-red-500" : "fill-neutral-400/30"}`}>
